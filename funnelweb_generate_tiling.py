...
def load_targets(catalogue, ra_min, ra_max, dec_min, dec_max, gal_lat_limit,
                 surveys=None, surveys_folder=None, priority_normal=2, 
    ...
    if surveys:
        start = time.time()
        priorities = import_all_surveys(surveys, surveys_folder)
        delta = time.time() - start
    ...
    for star in tabdata:
        ...
        if ((ra_min < star["RA_ep2015"] < ra_max)
            ...
            [priority, survey, reobserve] = get_target_priority(
                                                star["Gaia_ID"], 
                                                priorities, 
                                                priority_normal)
            ...
            if is_standard(star, use_colour_cut, standard_frac):
...
def is_standard(star, use_colour_cut=False, standard_frac=0.1, 
                colour_index_cut=0.5):
    ...
    if use_colour_cut:
        ...
        if (int(star["Gaia_G_mag"]*100) % int(1/standard_frac)) == 0:
            is_standard = True    
        else:
            is_standard = False
...
def get_target_priority(target_id, target_priorities=None, normal_priority=2):
    """Attempts to lookup the provided target ID in the list of survey target
    priorities, returning the priority and survey details if found. Otherwise,  
    returns the accepted "normal" priority and defaults.
    
    TODO: This function eventually needs to be able to check to see if targets
    have already been observed, and reduce their initial priority if they are
    not being considered for reobservation. The expectation is that this will 
    take the form of a single fits file, mapping IDs to whether the target 
    should go to priority 1 or 0 (details TBD).
    ...
    target_priorities: dict
        Dictionary mapping target IDs to list of form: 
        [priority, [survey1,..], reobserve].
    normal_priority: int
    ...
    if not target_priorities:
        return [normal_priority, ["None"], False]
    
    # We have a dictionary of priorities, check for membership    
    if target_id in target_priorities:
        # TODO Also check to see if the target has been observed    
        return target_priorities[target_id]
    else:
        return [normal_priority, ["None"], False]   
         


def import_all_surveys(surveys, surveys_folder):
    """Function to import all lists of priority and supplementary survey 
    targets, as recorded in fwts.surveys.
    
    Parameters
    ----------
    surveys: dict
        Dictionary of surveys, mapping survey name to a list of its parameters
        in the following form: [priority, reobserve]
    surveys_folder: string
        The folder on disk where the surveys are stored, with the expectation
        that each is a .fits file within this directory.
        
    Returns
    -------
    all_survey_targets: dict
        Combined dictionary of all survey targets mapping target ID to a list
        of the following format: [priority, [survey1,...], reobserve].
    """
    # Initialise dictionary to record survey details and priorities of all 
    # survey targets
    all_survey_targets = {}
    
    survey_crossover_count = 0
    
    print "Importing priority and supplementary surveys..."
    
    for survey in surveys:
        # Load and format the targets for each survey
        survey_file = surveys_folder + survey + ".fits"
        survey_targets = import_survey_priorities(survey, survey_file, 
                                                  surveys[survey][0], surveys[survey][1])
        
        # Add each target to the overall dictionary, making sure to handle
        # targets in common between multiple surveys
        for target in survey_targets:
            if target not in all_survey_targets:
                all_survey_targets[target] = survey_targets[target]
            else:
                # Target is in multiple surveys, assign highest of two 
                # priorities, both survey names, and True if the target is to
                # be reobserved
                priority = max([all_survey_targets[target][0], 
                                survey_targets[target][0]])
                                
                names = (all_survey_targets[target][1] + 
                                survey_targets[target][1])
                                
                reobserve = max([all_survey_targets[target][2], 
                                survey_targets[target][2]])
                
                all_survey_targets[target] = [priority, names, reobserve]
                
                survey_crossover_count += 1
        
    # Now should have all survey targets imported. Return
    print "\n%i targets in priority/supp surveys" % len(all_survey_targets)
    print "%i targets in common between surveys" % survey_crossover_count
    return all_survey_targets
    

def import_survey_priorities(survey, survey_file, priority, reobserve):
    """Function to import target list for a single priority or supplementary
    survey.
    
    Parameters
    ----------
    survey: string
        The unique name of the survey.
    survey_file: string
        The filepath to the survey on disk in .fits format.
    priority: int
        The base priority for all targets in the survey.
    reobserve: boolean
        Whether the targets in this survey will be reobserved as part of their
        survey and thus will not have their tiling reduced if they have already
        been observed.
    
    Returns
    -------
    survey_targets: dict
        Dictionary of survey targets mapping target ID to a list
        of the following format: [priority, [survey], reobserve].
    """
    # Load table of survey stars and extract IDs as a set
    survey_table = Table.read(survey_file)
    ids = set(survey_table["gaia_id"])

    survey_targets = dict.fromkeys(ids)
    
    # Assign the survey priority, name, and reobservation status to each target    
    for target in survey_targets:
        survey_targets[target] = [priority, [survey], reobserve]
        
    print "\t%s, priority=%i, reobserve=%s" % (survey, priority, reobserve)
        
    return survey_targets
...
try:
    ...
    all_targets = load_targets(fwts.script_settings["input_catalogue"],
                               ...
                               fwts.tiler_input["gal_lat_limit"],
                               fwts.surveys, fwts.surveys_folder, 
